...
export default function Home(props) {
  ...
  useEffect(() => {
    ...
    const amounttotake=async()=>{
      const response = await fetch("http://localhost:5000/transact/totake", {
      ...
      const fetchalltransactions=async()=>{
        const response = await fetch("http://localhost:5000/transact/fetchalltransact", {
            method: 'POST', // *GET, POST, PUT, DELETE, etc.
            headers: {
              'authToken': localStorage.getItem('token')
              // 'Content-Type': 'application/x-www-form-urlencoded',
  ...
  return (
    ...
    <h3>Amount To Pay</h3>
    <div className="row">
    {(topay.length>0 && topay.length!==undefined)?topay.map((temp,index)=>{
    ...
    <div className="row">
    {(transaction.length>0)?transaction.map((temp,index)=>{
